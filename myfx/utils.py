...
def fetch_xauusd(latest_only=True):
    ...
    try:
        ...
        # ✅ Drop incomplete (future) candles and sort ascending by time
        now_utc = pd.Timestamp.now(tz="UTC")
        cleaned_values = []
        for v in values:
            dt_parsed = parse_datetime(v["datetime"])
            if not dt_parsed:
                continue
            dt_utc = dt_parsed.replace(tzinfo=pytz.UTC)
            if dt_utc <= now_utc:
                cleaned_values.append({**v, "_dt": dt_utc})
            else:
                print(f"Dropped incomplete candle at {dt_utc}")

        cleaned_values.sort(key=lambda x: x["_dt"])  # oldest -> newest

        if not cleaned_values:
            print("All candles were incomplete, nothing to save")
            return False

        # If latest_only, restrict to only candles newer than DB latest
        if latest_only:
            latest_entry = ForexData.objects.order_by('-date').first()
            if latest_entry:
                cleaned_values = [v for v in cleaned_values if v["_dt"] > latest_entry.date]
                if not cleaned_values:
