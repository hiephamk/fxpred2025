...
def predict_next_hours(hours=24, sequence_length=60):
    ...
    with torch.no_grad():
        for i in range(hours):
            ...
            # Persist using timezone-aware datetime
            dt_utc = pred_time_utc.to_pydatetime()
            
            # Save to database (idempotent by datetime)
            PredictedForexData.objects.update_or_create(
                date=dt_utc,
                defaults={
            ...
            predictions.append({
                "datetime": pred_time_utc.isoformat().replace("+00:00", "Z"),
                "predicted_close": round(pred_price, 2)
            })
